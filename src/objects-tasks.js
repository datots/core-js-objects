...
function makeWord(lettersObject) {
  ...
  letters.forEach((letter) => {
    const positionsArray = lettersObject[letter];
    positionsArray.forEach((position) => {
      if (!positionsMap[position]) {
        positionsMap[position] = [];
      }
      positionsMap[position].push(letter);
    });
  });

  const maxPosition = Math.max(...Object.keys(positionsMap).map(Number));
  const wordArray = Array.from({ length: maxPosition + 1 }, (_, i) =>
    (positionsMap[i] || []).join('')
  );
